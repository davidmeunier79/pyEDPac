...
def test_spikes():
    ...
    # N0 -> N1
    syn1 = EDSynapse(neurons[0], neurons[2], synapse_config)
    synapses.append(syn1)
    print(f"Created synapse: {syn1}")


    print(len(neurons[2].incoming_links))
    ...
    # NeuronTracer
    neuron_tracer = NeuronTracer(neurons[2], buffer_size=120, height=100)
    neuron_tracer.record(0)
    ...
    neurons[0].emit_spike(0)
    print(f"Init: Neuron 0 spike: {neurons[0].last_time_of_firing}")

    neurons[1].emit_spike(0)
    print(f"Init: Neuron 1 spike: {neurons[1].last_time_of_firing}")

    MAX_TIME = 100

    while EDSynapse.event_manager.get_time() < MAX_TIME and EDSynapse.event_manager.get_nb_events():
        ...
        if time % 5 == 0:
            ...
            # Input au neurone 0 à t=10ms
            neurons[0].emit_spike(time)
            print(f"Neuron 0 spike: {neurons[0].last_time_of_firing}")

            neurons[1].emit_spike(time)
            print(f"Neuron 1 spike: {neurons[1].last_time_of_firing}")
